...
- [快速开始（Docker Run）](#quickstart-run)
- [容器更新（升级到新版本）](#update)
- [配置说明](#config)
- [使用方式](#usage)
- [目录映射与持久化](#volumes)
...
## <a id="quickstart-run"></a>快速开始（Docker Run）

适合：不使用 Compose，只希望用一条命令启动并运行在后台。

### 1) 准备 `.env`

与上面 Docker Compose 方式一致，至少需要：
...
> 注意：镜像仓库里的 **用户名与项目名建议全部小写**（例如 `ghcr.io/yourname/teleflux:v1.1.1`）。

- GHCR：`ghcr.io/<用户名>/<项目名>:v1.1.1`
- Docker Hub：`<用户名>/<项目名>:v1.1.1`

下面以变量 `IMAGE` 举例，按你的实际仓库替换：

### 3) 拉取并启动

```bash
IMAGE=ghcr.io/<username>/<project>:v1.1.1
...
### C) 使用 Docker Run 更新（升级到新版本）

> 思路：**拉新镜像 → 停旧容器 → 删旧容器 → 用同样参数重建并启动**。
